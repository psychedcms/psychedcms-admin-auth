...
export function ResetPasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL.replace(/\/api$/, '')}/api/reset-password`, {
                method: 'POST',
            ...
            <Box sx={{
                ...
                <Card sx={{ minWidth: 400, maxWidth: 450 }}>
                    <CardContent sx={{ p: 4, textAlign: 'center' }}>
                        <Typography color="error">{translate('psyched.auth.invalid_token')}</Typography>
                        <Box sx={{ mt: 2 }}>
            ...
            <Card sx={{ minWidth: 400, maxWidth: 450 }}>
                <CardContent sx={{ p: 4 }}>
                    <Typography variant="h5" gutterBottom align="center">
                        {translate('psyched.auth.reset_password_title')}
                    </Typography>

                    {success ? (
                        <>
                            <Alert severity="success" sx={{ mt: 2, mb: 2 }}>
                                {translate('psyched.auth.reset_password_success')}
                            </Alert>
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <Button
                                ...
                                size="large"
                            >
                                {translate('psyched.auth.reset_password_submit')}
                            </Button>
